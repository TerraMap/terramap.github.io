...
function readWorldFile(reader: DataStream, world: any): void {
    let position = 0;
...
function readFileFormatHeader(reader: DataStream, world: any): number[] {
    self.postMessage({
...
function readHeader(reader: DataStream, world: any): void {
...
function readTiles(reader: DataStream, world: any): void {
    self.postMessage({
...
function readChests(reader: DataStream, world: any): void {
    const chests: any[] = [];
...
function readSigns(reader: DataStream, world: any): void {
    const signs: any[] = [];
...
function getNpcType(id: number): string {
    const npc = settings.Npcs.find((element: any) => element.Id === id);
...
function readNpcs(reader: DataStream, world: any): void {
    const npcs: any[] = [];
...
function readTileEntity(reader: DataStream, world: any): any | null {
    const tileType = reader.readUint8();
    if (tileType < 0 || tileType > 10) {
      console.error({tileType});
        return null;
    }
    const tileEntity: any = {};
...
function readTileEntities(reader: DataStream, world: any): void {
    const byPosition = new Map<any, any>();
...
export { readString, readFileFormatHeader, readHeader, readWorldFile, readTiles, readChests, readSigns, readNpcs, readTileEntities };
